...
{
    [WorkFlowEventCaption("加班:登记")]
    public class OvertimeReg : DefaultStepEvent
    {
        /// <summary>
        /// 保存数据方法
        /// </summary>
        public override BoolMessage OnSaveData(FlowEventArgs args)
        {
            ...
            var entity = new Entity.EmployeeOvertime();
            ...
            {
                args.ExecuteData.Title = entity.Title = $"{args.CurrentUser.Name}的加班申请({DateTimeHelper.FormatDate(entity.CreateDateTime)})";
            }
            if (entity.Id == 0)
            {
