...
namespace Zeniths.Utility
{
    ...
    public class ChinaCalendar
    {
        ...
        public ChinaCalendar(DateTime dt)
        {
            int i;

            if (dt < MinDay || dt > MaxDay)
            {
                throw new InvalidOperationException("超出可转换的日期");
            }
            ...
            int leap = GetChineseLeapMonth(_cYear); //计算该年闰哪个月
            //设定当年是否有闰月
            _cIsLeapYear = leap > 0;
        ...
        public ChinaCalendar(int cy, int cm, int cd, bool leapMonthFlag)
        {
            ...
            for (int i = MinYear; i < cy; i++)
            {
                int temp = GetChineseYearDays(i); //求当年农历年天数
                offset = offset + temp;
            }
                ...
                (cm < leap)) //计算月份小于闰月     
            {
                ...
                for (int i = 1; i < cm; i++)
                {
                    int temp = GetChineseMonthDays(cy, i); //计算非闰月天数
                    offset = offset + temp;
                }
                ...
                {
                    throw new InvalidOperationException("不合法的农历日期");
                }
            ...
            else //是闰年，且计算月份大于或等于闰月
            {
                ...
                if (cm > leap) //计算月大于闰月
                {
                    int temp = GetChineseLeapMonthDays(cy); //计算闰月天数
                ...
                else //计算月等于闰月
                {
                    ...
                    {
                        int temp = GetChineseMonthDays(cy, cm); //计算非闰月天数
                        offset = offset + temp;
                    }
        ...
        private string GetChineseHour(DateTime dt)
        {
            ...
            return tmpGan[offset] + ZhiStr[offset].ToString(CultureInfo.InvariantCulture);
        }
        ...
        private void CheckChineseDateLimit(int year, int month, int day, bool leapMonth)
        {
            ...
            {
                throw new InvalidOperationException("非法农历日期");
            }
            if ((month < 1) || (month > 12))
            {
                throw new InvalidOperationException("非法农历日期");
        ...
        {
            get { return "公元" + this._date.ToLongDateString(); }
        }
        ...
        {
            get { return DateTime.IsLeapYear(this._date.Year); }
        }
        ...
        {
            get { return MonthString[this._cMonth]; }
        }
        ...
        /// 取农历日期表示法：农历正月初五
        /// </summary>
        public string ChineseMonthDateString
        {
            get
            {
                if (_cIsLeapMonth)
                {
                    return "农历" + "闰" + ChineseMonthString + ChineseDayString;
                }
                return "农历" + ChineseMonthString + ChineseDayString;
            }
        }
        ...
        public string GanZhiYearString
        {
            get
            {
                ...
                string tempStr = GanStr[i%10].ToString() + ZhiStr[i%12] + "年";
                return tempStr;
            }
        }
        ...
        public string GanZhiMonthString
        {
            get
            {
                ...
                int zhiIndex = _cMonth > 10 ? _cMonth - 10 : _cMonth + 2;
                string zhi = ZhiStr[zhiIndex - 1].ToString();
                ...
                string gan = GanStr[(ganIndex + this._cMonth - 2)%10].ToString();
        ...
        /// </summary>
        /// <returns></returns>
        public ChinaCalendar NextDay()
        {
            DateTime nextDay = _date.AddDays(1);
            return new ChinaCalendar(nextDay);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ChinaCalendar PervDay()
        {
            DateTime pervDay = _date.AddDays(-1);
            return new ChinaCalendar(pervDay);
        }
