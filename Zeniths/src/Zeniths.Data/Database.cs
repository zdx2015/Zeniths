...
namespace Zeniths.Data
{
    ...
    public class Database : IDisposable
    {
        ...
        public Database(string name)
        {
            ...
            if (setting == null)
            {
                throw new InvalidOperationException("无效的配置名称");
            }
        ...
        /// </summary>
        /// <param name="connection">连接对象</param>
        /// <param name="storeProcDefine">存储过程定义</param>
        /// <returns>命令对象</returns>
        public DbCommand CreateCommand(DbConnection connection, StoreProcDefine storeProcDefine)
        {
            DbCommand cmd = CreateCommand();
            if (storeProcDefine.Args != null)
            {
                // 参数解析
                var args_dest = new List<KeyValuePair<string, object>>();
                ParameterHelper.ParserStoreProcParameter(storeProcDefine.Args, args_dest);
                foreach (var item in args_dest)
                {
                    AddParam(cmd, item.Key, item.Value);
                }
            ...
            cmd.CommandText = storeProcDefine.Name;
        ...
        /// <param name="storeProcDefine">存储过程定义</param>
        /// <returns>命令对象</returns>
        public DbCommand CreateCommand(StoreProcDefine storeProcDefine)
        {
            return CreateCommand(null, storeProcDefine);
        }
        ...
        /// 执行命令并返回影响的行数
        /// </summary>
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回受影响的行数</returns>
        public int Execute(StoreProcDefine storeProcDefine)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, storeProcDefine))
                    {
                        _provider.PreExecute(cmd);
                        DoPreExecute(cmd);
                        OnExecutingCommand(cmd);
        ...
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回第一行第一列数据</returns>
        public T ExecuteScalar<T>(StoreProcDefine storeProcDefine)
        {
            try
            {
                OpenSharedConnection();
        ...
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回DbDataReader对象</returns>
        public DbDataReader ExecuteReader(StoreProcDefine storeProcDefine)
        {
            try
            {
                bool wasClosed = _sharedConnectionDepth == 0;
        ...
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回新创建的数据集</returns>
        public DataSet ExecuteDataSet(StoreProcDefine storeProcDefine)
        {
            try
            {
                OpenSharedConnection();
        ...
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回新创建的数据表</returns>
        public DataTable ExecuteDataTable(StoreProcDefine storeProcDefine)
        {
            var ds = ExecuteDataSet(storeProcDefine);
            return ds.Tables.Count > 0 ? ds.Tables[0] : null;
        }
        ...
        /// <param name="storeProcDefine">存储过程</param>
        /// <returns>返回记录集合</returns>
        public IEnumerable<T> Query<T>(StoreProcDefine storeProcDefine)
        {
            OpenSharedConnection();
            try
            {
